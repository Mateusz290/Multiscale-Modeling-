...
//        System.out.println("generate");
    ...
    public int generateWithBoundaries(){
        ...
        for(int i=0;i<x;i++){
            for(int j=0;j<y;j++){
                ...
                /*if (!grains[i][j].getGrainColor().equals(Color.GRAY)) {  // 3 - dolna podstawa    2 - prawa ściana     1 - górna podstawa   4 - lewa ściana
                    temp.setStyle("-fx-border-color: black; -fx-border-width: 3px 0px 0px 0px");
                }*/

                System.out.println("Akutalna pozycja to grains[" + i + "][" + j + "]");

                if (i + 1 < x) {

                    if (!grains[i][j].getGrainColor().equals(grains[i + 1][j].getGrainColor())) {
                        System.out.println("Prawy element ma inny kolor. Tworzenie prawego obramowania");
                        temp.setStyle("-fx-border-color: black; -fx-border-width: 0px 3px 0px 0px");
                        first = true;
                    }
                }
                ...
                if (j+1 < y) {
                    ...
                    if (!grains[i][j].getGrainColor().equals(grains[i][j + 1].getGrainColor())) {
                        System.out.println("Dolny element ma inny kolor. Tworzenie dolnego obramowania");

                        if (first) {
                            temp.setStyle("-fx-border-color: black; -fx-border-width: 0px 3px 3px 0px");
                            second = true;
                        }
                        else temp.setStyle("-fx-border-color: black; -fx-border-width: 0px 0px 3px 0px");
                    }
                    first = false;
                }
                ...
                if (i-1>=0) {
                    ...
                    if (!grains[i][j].getGrainColor().equals(grains[i-1][j].getGrainColor())) {
                        ...
                        if (second) {
                            temp.setStyle("-fx-border-color: black; -fx-border-width: 0px 0px 3px 3px");
                            second = true;
                        } else if (first) {
                            temp.setStyle("-fx-border-color: black; -fx-border-width: 0px 3px 3px 0px");

                        } else if(!second && !first) temp.setStyle("-fx-border-color: black; -fx-border-width: 0px 0px 3px 0px");
                    }
                    first = false;
                    second = false;
                }
...
//                temp.setStyle("-fx-border-color: green; -fx-border-width: 0px 0px 3px 0px");
                ret.getChildren().add(temp);
        ...
        int length = getBoundariesLength();

        controller.output.getChildren().clear();
        controller.output.getChildren().add(ret);
        return length;
    }


    public int getBoundariesLength() {
        int length = 0;
        size =(int) scale * 700 / x;
        for(int i=0;i<x;i++) {
            for (int j = 0; j < y; j++) {
    ...
    public AnchorPane createBoundary(){
        AnchorPane a=new AnchorPane();
        a.setBackground(new Background(new BackgroundFill(Color.GREY, CornerRadii.EMPTY, Insets.EMPTY)));

        return a;
    }

    public Map<Paint,Integer> getGrainsSize() {

        Map<Paint, Integer> grainsSize = new HashMap<>();
        Map<Paint, Integer> grainsSizeMedium = new HashMap<>();

        Map<Paint, Integer> grainsMinLength = new HashMap<>();
        Map<Paint, Integer> grainsMaxLength = new HashMap<>();

        Map<Paint, Integer> grainsMinLength2 = new HashMap<>();
        Map<Paint, Integer> grainsMaxLength2 = new HashMap<>();

        Map<Paint, Integer> grainsLengthX = new HashMap<>();
        Map<Paint, Integer> grainsLengthY = new HashMap<>();




        for (Paint paint: alive) {
            grainsSize.put(paint, 0);
            grainsSizeMedium.put(paint,0);
            grainsMinLength.put(paint,0);
            grainsMaxLength.put(paint,0);
            grainsMaxLength2.put(paint,0);
            grainsMinLength.put(paint,0);
            grainsLengthX.put(paint,0);
            grainsLengthY.put(paint,0);
...
//        System.out.println("Kolor w pierwszym elemencie " + grains[0][0].getGrainColor());

//        System.out.println("Mapa zawiera klucz? " + grainsSize.get(grains[0][0].getGrainColor()));

        for(int i=0;i<x;i++) {
            for (int j = 0; j < y; j++) {
                System.out.println("grains[" + i + "][" + j + "]") ;
                grainsSize.put(grains[i][j].getGrainColor(), grainsSize.get(grains[i][j].getGrainColor()) + 1);
                grainsMinLength.put(grains[i][j].getGrainColor(), min(grainsMinLength.get(grains[i][j].getGrainColor()),grains[i][j].getX()));
                grainsMaxLength.put(grains[i][j].getGrainColor(), max(grainsMinLength.get(grains[i][j].getGrainColor()),grains[i][j].getX()));

                grainsMaxLength2.put(grains[i][j].getGrainColor(), max(grainsMinLength.get(grains[i][j].getGrainColor()),grains[i][j].getY()));
                grainsMinLength2.put(grains[i][j].getGrainColor(), min(grainsMinLength.get(grains[i][j].getGrainColor()),grains[i][j].getY()));
        ...
        for (Paint key : grainsSize.keySet()) {
            System.out.println("Kolor: " + key);
        }

        Set<Map.Entry<Paint,Integer>> entrySet = grainsSize.entrySet();
        for(Map.Entry<Paint, Integer> entry: entrySet) {
            System.out.println("Klucz: " +entry.getKey() + " wartosc : " + entry.getValue());
            grainsSizeMedium.put(entry.getKey(), entry.getValue()/ alive.size());
        }

        System.out.println();
        System.out.println("Srednia dlugosc ziaren");

        Set<Map.Entry<Paint,Integer>> entrySet2 = grainsSizeMedium.entrySet();
        for(Map.Entry<Paint, Integer> entry: entrySet2) {
            System.out.println("Klucz: " +entry.getKey() + " wartosc : " + entry.getValue());
            grainsSizeMedium.put(entry.getKey(), entry.getValue()/ alive.size());
        }
    ...
    public Map<Paint,Double> getGrainsMediumSize() {

        Map<Paint, Integer> grainsSize = new HashMap<>();
        Map<Paint, Double> grainsSizeMedium = new HashMap<>();


        for (Paint paint: alive) {
            grainsSize.put(paint, 0);
            grainsSizeMedium.put(paint,0.0);
        }
...
//        System.out.println("Mapa zawiera klucz? " + grainsSize.get(grains[0][0].getGrainColor()));
        ...
        for(Map.Entry<Paint, Integer> entry: entrySet) {
            System.out.println("Klucz: " +entry.getKey() + " wartosc : " + entry.getValue());
            grainsSizeMedium.put(entry.getKey(), entry.getValue()/ (double) alive.size());
        }

        System.out.println();
        System.out.println("Srednia dlugosc ziaren");

        Set<Map.Entry<Paint,Double>> entrySet2 = grainsSizeMedium.entrySet();
        for(Map.Entry<Paint, Double> entry: entrySet2) {
            System.out.println("Klucz: " +entry.getKey() + " wartosc : " + entry.getValue());
            grainsSizeMedium.put(entry.getKey(), entry.getValue()/ (double) alive.size());
        }


        return grainsSizeMedium;
    }
...
//            controller.output.getChildren().add(this.generate());
    ...
    public void saveToGrainSizeToFile(PrintWriter printGrainSizeFile, Map<Paint, Integer> grainSizes) {

        Set<Map.Entry<Paint,Integer>> entrySet = grainSizes.entrySet();
        for(Map.Entry<Paint, Integer> entry: entrySet) {
            System.out.println("Klucz: " +entry.getKey() + " wartosc : " + entry.getValue());
            printGrainSizeFile.print(entry.getKey() + " " + entry.getValue());
            printGrainSizeFile.println();
        }

    }

    public void saveToMediumGrainSizeToFile(PrintWriter printGrainSizeFile, Map<Paint, Double> grainSizes) {

        Set<Map.Entry<Paint,Double>> entrySet = grainSizes.entrySet();
        for(Map.Entry<Paint, Double> entry: entrySet) {
            System.out.println("Klucz: " +entry.getKey() + " wartosc : " + entry.getValue());
            printGrainSizeFile.print(entry.getKey() + " " + entry.getValue());
            printGrainSizeFile.println();
        }
